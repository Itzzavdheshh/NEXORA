...
const STATUS_CONFIG = {
  ...
  completed: { label: "Completed", pill: "bg-amber-500/12 text-[var(--accent-primary)]"},
...
export function BookingCard({ booking, onStatusUpdate, isUpdating, onSelect, index }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex cursor-pointer flex-col justify-between rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5 transition duration-200 hover:-translate-y-0.5 hover:border-[var(--border-strong)]"
    >
      ...
      {/* Notes preview */}
      {notes && (
        <p className="mt-3 line-clamp-1 text-[11px] italic text-[var(--text-tertiary)]">
          "{notes}"
        </p>
      )}
